    ...
    if df_canvas is not None:
        ...
        if es_submissions:
            ...
            if st.button("🔍 Calcular Deudores Reales", type="primary"):
                if not archivo_xlsx:
                    st.error("⚠️ El reporte detectado es de **Submissions (Entregas)**. Para procesar este formato, el archivo '2. Base de Alumnos (XLSX)' es estrictamente obligatorio.")
                else:
        ...
        # ==========================================
            ...
            ejecutar_calculo = st.button("🔍 Calcular Deudores Reales (Calificaciones)", type="primary")
            ...
            if ejecutar_calculo:
                if not archivo_xlsx:
                    if opcion_base == "Base para HubSpot":
                        ...
                        if config_template and not df_final_wsp.empty:
                            columnas_salida = ['dni']
                            for p in sorted(config_template.keys(), key=int):
                                col_p = f"param_{p}"
                                if config_template[p]["tipo"] == "✍️ Texto Fijo":
