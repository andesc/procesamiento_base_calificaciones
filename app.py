...
MATERIAS_EXCLUIR_API = [
    "ADMINISTRACION GENERAL DE LA EMPRESA AGRARIA",
    "CEREMONIAL Y PROTOCOLO",
    "CIBERCAPACIDADES",
    "COMERCIALIZACION Y REVENUE MANAGEMENT",
    ...
    "LEARNING AGILITY",
    "MULTIMEDIOS",
    "TOMA DE DECISIONES PARA LA ACCION",
    "ALIMENTOS BEBIDAS Y EVENTOS",
    "DISENO DE SERVICIO AL CLIENTE",
    "GESTION DE CULTIVOS EXTENSIVOS",
    "RESOLUCION DE PROBLEMAS",
    "COMUNICACION EFECTIVA",
    "ORGANIZACION DEL TIEMPO Y DEL TRABAJO",
    "MATEMATICA Y ESTADISTICA",
    "PROCESO Y ESTRATEGIA DE MEJORA",
    "GESTION DE PROYECTOS"
]

# Normalizamos la lista negra de antemano
...
with st.expander("📌 Instrucciones de uso - LEER AQUÍ"):
    if opcion_base == "Base para HubSpot":
        ...
        **Para generar la Base de WhatsApp (Segmentada de a 100):**
        ...
        3. **Importante:** Estos archivos parciales para WhatsApp se descargan **sin encabezados** para que puedas copiarlos y pegarlos directamente en tus herramientas de envío masivo.
        """)
...
if archivo_csv and archivo_xlsx:
    try:
        # Lectura tolerante a fallos de codificación y separadores comunes (, o ;)
        ...
        if 'sis user id' in cols_canvas_lower and 'assignment name' in cols_canvas_lower:
            df_canvas.columns = cols_canvas_lower
            st.success("📂 **Reportes detectados y cargados correctamente.**")
            ...
            # Selector dinámico basado en las materias del Excel
            ...
            if st.button("🔍 Calcular Deudores Reales", type="primary"):
                # Filtrar universo objetivo inicial
                ...
                # --- NUEVA REGLA: EXCLUSIÓN AUTOMÁTICA DE MATERIAS SI ES UNA API ---
                if "API" in actividad_objetivo.upper():
                    # Filtramos sacando las que coincidan con la lista negra usando el texto normalizado
                    df_universo = df_universo[~df_universo['materia_match'].isin(LISTA_NEGRA_LIMPIA)]
                
                # Llave única compuesta Alumno-Materia
                df_universo['llave_cruce'] = df_universo['id_match'] + "_" + df_universo['materia_match']
                
                # Alumnos que SÍ entregaron de forma válida en Canvas
                ...
                # Exclusión quirúrgica: Alumnos del universo que NO están en la lista de cumplidores
                df_deudores = df_universo[~df_universo['llave_cruce'].isin(lista_cumplidores)].copy()
                
                # Formateo estético final
                ...
                # Si vas a usar WhatsApp, incluimos la columna celular si está disponible
                ...
                st.session_state.df_resultado = df_deudores[columnas_salida].rename(
                    columns=columnas_renombre
            ...
            st.warning("⚠️ El archivo CSV cargado no parece ser un reporte de entregas (Submissions) válido de Canvas.")
        ...
        if st.session_state.procesado:
            ...
            if opcion_base == "Base para HubSpot":
                ...
                # Particionado automático para WhatsApp de a 100 filas sin encabezado
                grid = st.columns(3)
