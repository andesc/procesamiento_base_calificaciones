...
# --- SECCIÓN NUEVA: CONFIGURACIÓN DINÁMICA DEL TEMPLATE DE META (SOLO PARA WHATSAPP) ---
config_parametros = {}
if opcion_base == "Base para Whatsapp" and archivo_csv:
    ...
    # Detectar cuántos {{X}} existen en el texto
    import re
            ...
            if not archivo_xlsx:
                ...
                if st.button("🔍 Calcular Deudores Reales", type="primary"):
                    ...
                    if opcion_base == "Base para HubSpot":
                        ...
                        # NUEVA LÓGICA DE SALIDA DINÁMICA PARA WHATSAPP (META)
            ...
            if ejecutar_calculo:
                if not archivo_xlsx:
                    if aplicar_exclusion and materia_archivo_limpia in LISTA_NEGRA_LIMPIA:
                        ...
                        df_final = df_canvas.drop_duplicates(subset=['dni'])
                    
                    if opcion_base == "Base para HubSpot":
                        st.session_state.df_resultado = pd.DataFrame(columns=['email'])
                    else:
                        # Salida dinámica sin Excel
                        columnas_salida = ['dni']
                        for param in sorted(config_parametros.keys(), key=int):
                            conf = config_parametros[param]
                            col_name = f"param_{param}"
                    ...
                    if opcion_base == "Base para HubSpot":
                        st.session_state.df_resultado = df_cruce[['email']].dropna().
